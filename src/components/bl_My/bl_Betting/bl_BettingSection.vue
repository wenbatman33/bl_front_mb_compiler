  ...
  .bettingSection.mb-2(@click="openCard")
    ...
    .d-flex
      .d-flex.flex-column.w-50
        .bettingSection__subTitle.mb-1 流水金額
        .bettingSection__subContent ¥{{ format(Math.floor((game.betAmount * 100) / 100).toFixed(2)) }}
      .d-flex.flex-column.w-50
        .bettingSection__subTitle.mb-1 输赢金额
        .bettingSection__subContent ¥{{ format(Math.floor((game.betWinAmount * 100) / 100).toFixed(2)) }}
</template>
