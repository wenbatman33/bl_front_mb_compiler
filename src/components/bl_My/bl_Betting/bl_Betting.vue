<template lang="pug">
  .betting
    .betting__header(:style="{ backgroundImage: 'url(' + bgImage + ')' }")
      router-link(to="/my")
        i.fas.fa-chevron-left
      p 我的投注
      v-date-picker(ref="calendar" :popover="{ placement: 'bottom', visibility: 'click' }" mode="range" v-model="dateValue" :available-dates='{ start: start_date, end: end_date }' focus:outline-none is-dark)
    ...
    .betting__function
      el-dropdown(@command="setType" trigger="click" placement="bottom")
        ...
        el-dropdown-menu(slot="dropdown")
          el-dropdown-item(v-for="item in type" :command="item") {{ item }}
      el-dropdown.w-100(@command="setGame" trigger="click" placement="bottom")
        .el-dropdown-link.betting__type.mr-0 {{ gameDisplay }}
          i.el-icon-caret-bottom.ml-1
        el-dropdown-menu(slot="dropdown")
          el-dropdown-item(v-for="item in games" :command="item") {{ item.name }}
    .betting__time
      i.fas.fa-clock.mr-2
      | {{this.formatDate(dateValue.start) }} ~ {{this.formatDate(dateValue.end) }}
...
export default {
  ...
  methods: {
    setType(type) {
      ...
      this.form.params.game_type = this.type.indexOf(type) + 1;
  ...
  watch: {
    dateValue: {
      handler(date) {
        ...
        this.form.params.time_end = this.formatDate(date.end);
        this.$store.dispatch('member/API_IndexMemberBetList', this.form).then((res) => {
          this.bettingLog = res.data.data;
          this.currentPage = res.data.meta.pagination.current_page;
          this.totalPages = res.data.meta.pagination.total_pages;
