<template lang="pug">
  .transactionLog
    .transactionLog__header(:style="{ backgroundImage: 'url(' + bgImage + ')' }")
      router-link(to="/my")
    ...
    .transactionLog__function
      el-dropdown(@command="setType" trigger="click")
        ...
        el-dropdown-menu(slot="dropdown")
          el-dropdown-item(v-for="(item, index) in typeList" :key="item" :command="item") {{ index }}
      .transactionLog__datepicker.p-2
        i.fas.fa-clock.mr-1
        v-date-picker(ref="calendar" mode="range" v-model="dateValue" color="red" :input-props='{class: "py-0 w-100"}' :available-dates='{ start: start_date, end: end_date }' is-dark)
    .transactionLog__main(v-infinite-scroll="load")
      ...
      div(v-else v-for="(item, index) in filterTransactionLog" :key="index")
        ...
        TransactionLogSection(v-for="log in item" :card="log" :key="log.orderId")
</template>
...
export default {
  ...
  computed: {
    date() {
      if (this.dateValue !== null) {
        const temp = {
          start_time: this.formatDate(this.dateValue.start),
          end_time: this.formatDate(this.dateValue.end),
        };
        return temp;
      }
    },
  ...
  watch: {
    dateValue: {
      handler(val) {
        this.filterForm.params.start_time = this.date.start_time;
        this.filterForm.params.end_time = this.date.end_time;
        this.init();
        this.$store.dispatch('payment/API_MemberTransactionLog', this.filterForm).then((res) => {
          this.transactionLog = res.data.data;
          this.currentPage = res.data.meta.pagination.current_page;
