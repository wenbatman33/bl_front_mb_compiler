<template lang="pug">
  ...
  .money
    .money__header
      ...
      .money__header__tag.col(:class="{'active1': activeTag}" @click="changeToDeposit") 存款
      .money__header__tag.col(:class="{'active2': !activeTag}" @click="changeToWithdraw") 提款
    ...
    .deposit__main(v-if="activeTagName === 'Deposit'")
      div(v-if="depositStep === 1")
        ...
        el-input.deposit__main__moneyInput(type="tel" placeholder="请输入存款金额" v-model="depositForm.amount")
      ...
      div(v-else-if="depositStep === 2 && returnType === 'qrcode'")
        ...
        p.text-center 请使用 <span class="text-danger">银联</span> 扫描二维码
    ...
    .withdraw__main(v-else-if="activeTagName === 'withdraw'")
      ...
      .withdraw__main__moneyInput
        el-input(v-model.number="withdrawForm.amount" placeholder="请输入提款金额")
        el-button(type="info" plain) 最高
      .withdraw__main__title
        p 开户名
      el-input.withdraw__main__nameInput(v-model="IndexPersonal.memberRealname" placeholder="请输入用戶名" disabled)
      .withdraw__main__title
        p 支付银行
        el-button(type="text" @click="addBankCard") 添加银行卡
...
export default {
  ...
  methods: {
    changeToDeposit() {
      this.activeTag = true;
      this.activeTagName = 'Deposit';
    },
    changeToWithdraw() {
      this.activeTag = false;
      this.activeTagName = 'withdraw';
    },
    ...
    deposit() {
      this.$store.dispatch('payment/API_Deposit', this.depositForm).then((res) => {
        ...
        if (res.data.data.type === 'redirect') {
          ...
          this.returnContent = this.returnContent.replace(/target='_blank'/gm, 'target="_self"')
          setTimeout(() => {
    ...
    addBankCard() {
      if (window.navigator.userAgent.indexOf('Android') > 0) {
        try {
          window.android.app_addBankCard();
        } catch (error) {
          console.log(error);
        }
      } else {
        try {
          window.webkit.messageHandlers.app_addBankCard.postMessage('app_addBankCard');
        } catch (error) {
          console.log(error);
        }
      }
    ...
    toDepositLog() {
      if (window.navigator.userAgent.indexOf('Android') > 0) {
        try {
          window.android.deposit_log();
        } catch (error) {
          console.log(error);
        }
      } else {
        try {
          window.webkit.messageHandlers.deposit_log.postMessage('deposit_log');
        } catch (error) {
          console.log(error);
        }
      }
    ...
    toWithdrawLog() {
      if (window.navigator.userAgent.indexOf('Android') > 0) {
        try {
          window.android.withdraw_log();
        } catch (error) {
          console.log(error);
        }
      } else {
        try {
          window.webkit.messageHandlers.withdraw_log.postMessage('withdraw_log');
        } catch (error) {
          console.log(error);
        }
      }
    ...
    // for appteam methods
    app_addBankCard() {
      console.log('app_addBankCard');
    },
    // 資金紀錄 - 存款
    deposit_log() {
      console.log('deposit_log');
    },
    // 資金紀錄 - 提款
    withdraw_log() {
      console.log('withdraw_log');
    },
    token_expired() {
  ...
  mounted() {
    ...
    window.app_addBankCard = this.app_addBankCard;
    window.deposit_log = this.deposit_log;
    window.withdraw_log = this.withdraw_log;
  },
};
</script>
