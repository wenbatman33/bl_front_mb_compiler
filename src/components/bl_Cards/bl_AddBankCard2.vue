  ...
  .bankManage
    ...
    .bankManage__main
      ...
      el-form(v-model="addBankForm")
        el-form-item
          el-input.bankManage__main__input(placeholder='开户名' v-model="addBankForm.bank_account" :disabled="IndexPersonal.memberRealname !== null")
        el-form-item
          el-select.w-100.bankManage__main__input(v-model="addBankForm.bank_code" placeholder='请选择开户行')
        ...
        el-form-item
          .d-flex
            el-select.bankManage__main__input(v-model="addBankForm.provinceValue" placeholder='省')
              el-option(v-for="(item, index) in IndexProvinceList" :key="index" :label="item[0].province" :value="index")
            el-select.bankManage__main__input.ml-1.mr-1(v-model="addBankForm.city" placeholder='市')
              el-option(v-for="(item, index) in IndexCityList" :key="item.id" :label="item.name" :value="item.name")
