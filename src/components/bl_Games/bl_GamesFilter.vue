<template lang="pug">
  .filter
    .filter__header
      ...
      | 进阶筛选
      el-button(type="text") 重置
    .filter__default
      .filter__default__block
        .filter__default__block--title 游戏分类
        .filter__default__block--items
          button.mr-2(v-for="item in gameTagList(1)" :key="item.id" @click="select(item.id)" :class="{'activeTag': tagActive(item.id)}") {{ item.gameTagName }}
      .filter__default__block
        .filter__default__block--title 游戏线数
        .filter__default__block--items
          button.mr-2(v-for="item in gameTagList(2)" :key="item.id" @click="select(item.id)" :class="{'activeTag': tagActive(item.id)}") {{ item.gameTagName }}
    .filter__main
